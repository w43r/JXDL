(function(H) {
     ...
     * @param  {Boolean}   [isJPEG]   是否为JPEG格式，默认为PNG格式
     */
    H.Chart.prototype.convertImage = function(callback, isJPEG) {
        if (typeof callback !== 'function') {
            throw new Error("Callback Function is NOT Correct.");
        }
        // 图片格式
        var type = isJPEG ? MIME_TYPES.JPEG : MIME_TYPES.PNG;
        var options = {
            type: type,
            filename: "convert" + MIME_TYPE_TO_EXTENSION[type]
        };

        var preRender = preRenderImage,
            render = renderPngJpeg,
            highChartsObject = this;

        var context = preRender(highChartsObject, options);
        context.type = type;
        context.filename = options.filename;
        context.browserSupportDownload = browserSupportDownload;
        context.browserSupportBlob = browserSupportBlob;

        render(highChartsObject, context, function(data) {
            callback(data.datauri);
        });
    };
